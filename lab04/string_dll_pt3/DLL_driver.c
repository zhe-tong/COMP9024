...
bool action(dll lists[])
{
    ...
    } else if (strcasecmp(toks[0], "insertBefore") == 0) {
        if (tok < 3 || strlen(toks[1]) != 1) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER> <string> | <string>\n", toks[0]);
        } else {
            size_t i = 3;
            for (; i < tok && strcmp(toks[i], "|"); i++) *(toks[i]-1) = ' ';
            size_t j = i + 1;
            for (; j < tok && strcmp(toks[j], "|"); j++) *(toks[j]-1) = ' ';
            if (isupper(toks[1][0])) {
                DLLinsertBefore(lists[toks[1][0] - 'A'], toks[2], toks[i+1]);
            } else if (islower(toks[1][0])) {
                DLLinsertBefore(lists[toks[1][0] - 'a'], toks[2], toks[i+1]);
            } else if (isdigit(toks[1][0])) {
                DLLinsertBefore(lists[toks[1][0] - '0'], toks[2], toks[i+1]);
            } else {
                printf("Invalid arguments to \"%s\"\n", toks[0]);
                printf("Usage: DLL> %s <LETTER> <string> | <string>\n", toks[0]);
            }
        }
    } else if (strcasecmp(toks[0], "replace") == 0 || strcasecmp(toks[0], "insert") == 0) {
        if (tok < 3 || strlen(toks[1]) != 1) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER> <string> | <string>\n", toks[0]);
        } else {
            ...
            for (; j < tok && strcmp(toks[j], "|"); j++) *(toks[j]-1) = ' ';
            if (isupper(toks[1][0])) {
                DLLreplace(lists[toks[1][0] - 'A'], toks[2], toks[i+1]);
            } else if (islower(toks[1][0])) {
                DLLreplace(lists[toks[1][0] - 'a'], toks[2], toks[i+1]);
            } else if (isdigit(toks[1][0])) {
                DLLreplace(lists[toks[1][0] - '0'], toks[2], toks[i+1]);
            } else {
                printf("Invalid arguments to \"%s\"\n", toks[0]);
                printf("Usage: DLL> %s <LETTER> <string> | <string>\n", toks[0]);
            }
        }
    } else if (strcasecmp(toks[0], "insertAfter") == 0) {
        if (tok < 3 || strlen(toks[1]) != 1) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER> <string> | <string>\n", toks[0]);
        } else {
            ...
            for (; j < tok && strcmp(toks[j], "|"); j++) *(toks[j]-1) = ' ';
            if (isupper(toks[1][0])) {
                DLLinsertAfter(lists[toks[1][0] - 'A'], toks[2], toks[i+1]);
            } else if (islower(toks[1][0])) {
                DLLinsertAfter(lists[toks[1][0] - 'a'], toks[2], toks[i+1]);
            } else if (isdigit(toks[1][0])) {
                DLLinsertAfter(lists[toks[1][0] - '0'], toks[2], toks[i+1]);
            } else {
                printf("Invalid arguments to \"%s\"\n", toks[0]);
                printf("Usage: DLL> %s <LETTER> <string> | <string>\n", toks[0]);
            }
        }
    } else if (strcasecmp(toks[0], "removeBefore") == 0) {
        if (tok < 3 || strlen(toks[1]) != 1) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER> <string>\n", toks[0]);
        } else {
            for (size_t i = 3; i < tok; i++) *(toks[i]-1) = ' ';
            if (isupper(toks[1][0])) {
                DLLremoveBefore(lists[toks[1][0] - 'A'], toks[2]);
            } else if (islower(toks[1][0])) {
                DLLremoveBefore(lists[toks[1][0] - 'a'], toks[2]);
            } else if (isdigit(toks[1][0])) {
                DLLremoveBefore(lists[toks[1][0] - '0'], toks[2]);
            } else {
                printf("Invalid arguments to \"%s\"\n", toks[0]);
                printf("Usage: DLL> %s <LETTER> <string>\n", toks[0]);
            }
        }
    } else if (strcasecmp(toks[0], "remove") == 0) {
        if (tok < 3 || strlen(toks[1]) != 1) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER> <string>\n", toks[0]);
        } else {
            for (size_t i = 3; i < tok; i++) *(toks[i]-1) = ' ';
            if (isupper(toks[1][0])) {
                DLLremove(lists[toks[1][0] - 'A'], toks[2]);
            } else if (islower(toks[1][0])) {
                DLLremove(lists[toks[1][0] - 'a'], toks[2]);
            } else if (isdigit(toks[1][0])) {
                DLLremove(lists[toks[1][0] - '0'], toks[2]);
            } else {
                printf("Invalid arguments to \"%s\"\n", toks[0]);
                printf("Usage: DLL> %s <LETTER> <string>\n", toks[0]);
            }
        }
    } else if (strcasecmp(toks[0], "removeAfter") == 0) {
        if (tok < 3 || strlen(toks[1]) != 1) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER> <string>\n", toks[0]);
        } else {
            for (size_t i = 3; i < tok; i++) *(toks[i]-1) = ' ';
            if (isupper(toks[1][0])) {
                DLLremoveAfter(lists[toks[1][0] - 'A'], toks[2]);
            } else if (islower(toks[1][0])) {
                DLLremoveAfter(lists[toks[1][0] - 'a'], toks[2]);
            } else if (isdigit(toks[1][0])) {
                DLLremoveAfter(lists[toks[1][0] - '0'], toks[2]);
            } else {
                printf("Invalid arguments to \"%s\"\n", toks[0]);
                printf("Usage: DLL> %s <LETTER> <string>\n", toks[0]);
            }
