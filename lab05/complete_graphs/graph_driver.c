...
bool action(graph graphs[])
{
    ...
    } else if (strcasecmp(toks[0], "isComplete") == 0 || strcasecmp(toks[0], "Complete?") == 0) {
        if (tok != 2) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER>\n", toks[0]);
        } else if (strlen(toks[1]) != 1) {
            ...
            if (isupper(toks[1][0])) {
                printf("%s\n", g_isComplete(graphs[toks[1][0] - 'A']) ? "True" : "False");
            } else if (islower(toks[1][0])) {
                printf("%s\n", g_isComplete(graphs[toks[1][0] - 'a']) ? "True" : "False");
            } else {
                printf("Invalid arguments to \"%s\"\n", toks[0]);
    ...
    } else if (strcasecmp(toks[0], "makeComplete") == 0) {
        if (tok != 3) {
            printf("Invalid arguments to \"%s\"\n", toks[0]);
            printf("Usage: DLL> %s <LETTER> <Size>\n", toks[0]);
        } else if (strlen(toks[1]) != 1) {
            ...
            if (*endpt == '\0') {
                if (isupper(toks[1][0])) {
                    if (!graphs[toks[1][0] - 'A']) {
                        graphs[toks[1][0] - 'A'] = g_makeComplete(value);
                    } else {
                        printf("Invalid arguments to \"%s\"\n", toks[0]);
                        printf("Argument 1: Graph '%c' is already allocated\n", toks[1][0]);
                    }
