...
graph g_create(size_t size) {
    graph new = calloc(1, sizeof(struct Graph)); assert(new);
    new->size = size;
    new->V    = calloc(size, sizeof(struct Vertex)); assert(new->V);
    ...
    new->E = calloc(size, sizeof(struct Edge*)); assert(new->E);

    for (size_t i = 0; i < size; i++) {
        new->E[i] = calloc(size, sizeof(struct Edge)); assert(new->E[i]);
